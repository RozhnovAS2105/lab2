...
set <int> selectPipesByFilter(unordered_map<int, Pipe>& pipes) {
	...
	case 2:
		foundPipes = findByFilter(pipes, checkPipeByInRepair, (bool)GetCorrectNumber("В работе \n(0-не работает, 1-работает)", 0, 1));
		break;
	}
	return foundPipes;
}
...
void editPipeByFilter(unordered_map<int, Pipe>& pipes, set <int> foundPipes) {
	if (!foundPipes.empty()) {
		...
		case 1:
		{
			bool inRepair = (bool)GetCorrectNumber("Выберете новое состояние для всех труб \n(0-не работает, 1-работает):", 0, 1);
			for (int i : foundPipes)
		...
		case 2:
		{
			...
			bool inRepair = (bool)GetCorrectNumber("Выберете новое состояние для труб по фильтру \n(0-не работает, 1-работает)", 0, 1);
		...
		case 3:
		{
			bool inRepair = (bool)GetCorrectNumber("На какое хотите заменить? \n(0-не работает, 1-работает)", 0, 1);
